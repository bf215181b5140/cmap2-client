...
export default class OscClockController {
    ...
    constructor() {
        ...
        IPC.on('setOscClockSettings', (data) => {
            this.settings = data;
            this.start();
        });
    }
    ...
    private sendToChatbox() {
        ...
        BRIDGE.emit('sendOscMessage', new Message('/chatbox/input', formattedText, true));
    }
    ...
    private sendToAvatar() {
        ...
        this.settings?.avatarParameters.forEach(parameter => {
            ...
            BRIDGE.emit('sendOscMessage', new Message(parameter.path, value));
        });
    }
}
