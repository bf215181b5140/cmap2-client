...
export default class LovenseController extends LovenseService {
    ...
    constructor() {
        ...
        IPC.on('setLovenseSettings', (lovenseSettings: LovenseSettings) => this.lovenseSettings = lovenseSettings);
        IPC.on('getLovenseStatus', () => IPC.emit('lovenseStatus', this.lovenseStatus));
        IPC.on('sendLovenseToyCommand', (toyCommand: ToyCommand) => this.sendToyCommand(toyCommand));
        IPC.on('lovenseConnect', () => this.connect());
        IPC.on('lovenseDisconnect', () => this.disconnect());
        IPC.on('setToyCommandParameters', (toyCommandParameters: ToyCommandParameter[]) => this.setToyCommandParameters(toyCommandParameters));
        IPC.on('setToyCommandOscMessages', (toyCommandOscMessages: ToyCommandOscMessage[]) => this.setToyCommandOscMessages(toyCommandOscMessages));

        BRIDGE.on('vrcParameter', (vrcParameter: VrcParameter) => this.checkOscParametersForToyCommand(vrcParameter));
    }
    ...
    private updateLovenseStatus() {
        ...
        IPC.emit('lovenseStatus', this.lovenseStatus);
    ...
    private checkToyCommandForOscMessage(toyCommand: ToyCommand) {
        ...
        toyCommandOscMessages?.forEach(toyCommandOscMessage => {
            ...
            BRIDGE.emit('sendOscMessage', new Message(toyCommandOscMessage.parameterPath, value));
        });
    }
}
